# Liquid

\[Name\] is a liquid that would go into the blood of a patient and repair internal bleedings and other damages by softening up the patients damaged parts to a viscous liquid then bringing them together to then turn back to normal. It was created by [Matt]({{ site.baseurl }}\Lore\Fursonas\Zlime), a scientist working on a private unregulated project in his workplace laboratory.

To know how the body should be put together it used some of the patient's blood to “know” the correct position of everything through the DNA in the blood.

In its latest form it was way too efficient and not stable. It turned its latest tests whole body into a liquid like state. Since it was made to be made with Human blood to be tested on a person, when it tried to rebuild, it anthropomorphised [Zlime]({{ site.baseurl }}\Lore\Fursonas\Zlime) into a cat/human hybrid and since it wasn’t stable it kept Zlime’s body able to go back to the liquid state.